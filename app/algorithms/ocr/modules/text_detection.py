...
class TextDetector(object):
    ...
    def prepare(
        ...
        config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "config", "pp_det", "inference.yml")
    ...
    def __call__(self, img, use_slice=False):
        ...
        elapse = 0
        if img.shape[0] / img.shape[1] > 2 and img.shape[0] > self.limit_side_len and use_slice:
            start_h = 0
            end_h = 0
            while end_h <= img.shape[0]:
                ...
                elapse += sub_elapse
        elif img.shape[1] / img.shape[0] > 3 and img.shape[1] > self.args.det_limit_side_len * 3 and use_slice:
            start_w = 0
            end_w = 0
