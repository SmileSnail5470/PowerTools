...
class EMDFInpaint():
    ...
    def inpaint(self, image_path: str, mask: np.ndarray, dilate_num: int=2):
        """
        ...
        for region_mask in regions:
            ...
            for box in boxes:
                ...
                if self._tile_has_watermark(mask):
                    ...
                    pred = [img]
                pred = np.clip(pred[0] * mask + img * (1 - mask), -1.0, 1.0)
                crop_image = np.uint8((np.transpose(pred[0], (1, 2, 0)) + 1) / 2. * 255.)
                if need_resize:
