...
class RGVI(nn.Module):
    def __init__(self):
        ...
        self.sdi = SDI.from_pretrained(os.path.join(os.path.dirname(os.path.abspath(__file__)), "weights", "stable-diffusion-2-inpainting"), torch_dtype=torch.float32, cache_dir='../huggingface').to('cpu')
    ...
    def forward(self, imgs, neg_masks, pos_masks, res, prompt):
        ...
        for i in range(1, L):
            ...
            fw_flows[i - 1] = self.raft(2 * prev_img.cpu() - 1, 2 * curr_img.cpu() - 1)[-1].cpu()
            bw_flows[i] = self.raft(2 * curr_img.cpu() - 1, 2 * prev_img.cpu() - 1)[-1].cpu()
        ...
        fcnet_flows = [fcnet_fw_flows.cpu(), fcnet_bw_flows.cpu()]
        fcnet_inp_flows = self.fcnet.forward_bidirect_flow(fcnet_flows, fcnet_masks.cpu())
        fcnet_inp_flows = self.fcnet.combine_flow(fcnet_flows, fcnet_inp_flows, fcnet_masks.cpu())
        ...
        if 1 in masks[k].unique():
            ...
            generator = torch.Generator('cpu').manual_seed(2024)
        ...
        for i in range(L):
            if 1 in masks[i].unique():
                imgs[i:i + 1] = self.pfcnet(imgs[i:i + 1].cpu(), masks[i:i + 1].cpu()).cpu()
